...
class Reaction {
  public:
    ...
    Reaction(const std::vector<Component> &reactants, const std::vector<Component> &products, MoleculePtr catalyst = nullptr)
    ...
    void execute() {
        ...
        // Consume reactants
        for (auto &reactant : reactants_) { reactant.molecule->amount -= reactant.stoichiometry * maxReactions; }

        // Produce products
        for (auto &product : products_) { product.molecule->amount += product.stoichiometry * maxReactions; }
